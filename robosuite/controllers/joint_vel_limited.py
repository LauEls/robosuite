...
class JointVelocityLimitedController(Controller):
    """
    ...
    def __init__(
        ...
        **kwargs,  # does nothing; used so no error raised when dict is passed with extra terms used previously
        ...
        self.joint_max_angle = joint_max_angle
        self.joint_min_angle = joint_min_angle
    ...
    def set_goal(self, velocities):
        ...
        for i in range(len(velocities)):
            if self.joint_pos[i] > self.joint_max_angle[i] or self.joint_pos[i] < self.joint_min_angle[i]:
                velocities[i] = 0
